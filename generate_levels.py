    ...
    for i,b in enumerate(blocks):
        if b.orient == 'H':
            if b.x - 1 >= 0 and grid[b.y][b.x-1] is None:
                new = list(blocks); new[i] = Block(b.id,b.orient,b.length,b.x-1,b.y); result.append(new)
            if b.x + b.length < WIDTH and grid[b.y][b.x+b.length] is None:
                new = list(blocks); new[i] = Block(b.id,b.orient,b.length,b.x+1,b.y); result.append(new)
        else:
            if b.y - 1 >= 0 and grid[b.y-1][b.x] is None:
                new = list(blocks); new[i] = Block(b.id,b.orient,b.length,b.x,b.y-1); result.append(new)
